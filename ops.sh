...
Commands:
  ...
  pull-build     git fetch/reset hard origin/main, build client+server, restart service
...
case "$cmd" in
  ...
  health)
    ...
    ;;
  pull-build)
    run_ssh "set -e; cd '$DIR'; git fetch --all; git reset --hard origin/main; cd client && npm install && npm run build; cd ../server && npm install && npm run build; systemctl restart external-logger.service"
    ;;
  *)
    usage
    exit 1
